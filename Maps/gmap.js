...
// Try W3C Geolocation (Preferred)
  ...
  function handleNoGeolocation(errorFlag) {
    if (errorFlag == true) {
      ...
      alert("Your browser doesn't support geolocation. We've placed you in Hyderabad, India.");
      initialLocation = india;
    }
	}
    map.setCenter(initialLocation);
...
function getNames(directionResult) {
	...
	summaryPanel.innerHTML = '';
	//For Submission into db
	$('#startPointN').val(route.legs[0].start_address);
	$('#endPointN').val(route.legs[route.legs.length-1].end_address);
	for (i = 1; i < route.legs.length ; i++)
	{
		document.getElementById('wayPoint'+i+'N').value = route.legs[i].start_address;
	}
