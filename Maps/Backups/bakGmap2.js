  ...
  function placeBothMarkers()
  {
	...
	var srcLatLng, destLatLng, p1l,p2l,p3l;
	if(p1!='')
	{
		geocoder.geocode( { 'address': p1}, function(results, status) {
			if (status == google.maps.GeocoderStatus.OK) 
			{
				p1l=results[0].geometry.location;
				addMarker(p1l);
			}
			else
			{
	...
	if(p2!='')
	{
		geocoder.geocode( { 'address': p2}, function(results, status) {
			if (status == google.maps.GeocoderStatus.OK) 
			{
				p2l=results[0].geometry.location;
				addMarker(p2l);
			}
			else
			{
	...
	if(p3!='')
	{
		geocoder.geocode( { 'address': p3}, function(results, status) {
			if (status == google.maps.GeocoderStatus.OK) 
			{
				p3l=results[0].geometry.location;
				addMarker(p3l);
			}
			else
			{
  ...
  function addMarker(location,isSrcDest) {
...
//	$("#geocode_info").html(myRoute.start_address);
	start.innerHTML = myRoute.start_address;
	end.innerHTML = myRoute.end_address;
	dist.innerHTML = myRoute.distance.value/1000 + 'Km';
}
